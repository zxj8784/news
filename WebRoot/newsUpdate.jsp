...
<div id="main">
  ...
  <div id="opt_area">
    ...
    <form action="/News/news_update?cnid=${cnid }" method="post">
    
      <p>
        <label> 主题 </label>
        <select name="ntid" id="ntid" size="1">
          <c:forEach items="${topicList}" var="topic" varStatus="status" >
          <c:if test="${topic.tid==newsList.get(0).getNtid() }">
      ...
      <p>
        <label> 标题 </label>
        <input name="ntitle" id="ntitle" type="text" class="opt_input" value="${newsList[0].ntitle }" />
      </p>
      <p>
        <label> 作者 </label>
        <input name="nauthor" id="nauthor" type="text" class="opt_input" value="${newsList[0].nauthor }" />
      </p>
      <p>
        <label> 摘要 </label>
        <textarea name="nsummary" id="nsummary" cols="40" rows="3">${newsList[0].nsummary }</textarea>
      </p>
      <p>
        <label> 内容 </label>
        <textarea class="ckeditor"  name="ncontent"  id="ncontent" cols="70" rows="10">${newsList[0].ncontent }</textarea>
      </p>
      <p>
        <label> 上传图片 </label>        
